...
export const AdminDashboard: React.FC = () => {
 ...
 <div>
 <span className="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full bg-amber-500/10 text-amber-700 border border-amber-500/20 text-xs font-extrabold uppercase tracking-wider">
 <Building2 className="w-3.5 h-3.5" /> Bhopal Smart City Operations
 </span>
 <h1 className="text-3xl sm:text-4xl font-black text-[#10201C] tracking-tight mt-1">
 Good morning, Admin.
 </h1>
 <p className="text-xs sm:text-sm text-[#536761] font-medium">
 ...
 <Link to="/admin/queue" className="text-xs font-bold text-amber-600 hover:underline flex items-center gap-1">
 ...
 <section className="space-y-4">
 <h2 className="text-2xl font-black text-[#10201C] flex items-center gap-2">
 <Building2 className="w-6 h-6 text-[#053229] " /> Department Workload Overview
 </h2>
 ...
 return (
 <div key={dept.id} className="bg-white rounded-2xl border border-[#D6E2DE] shadow-sm p-6 rounded-3xl space-y-4 shadow-lg hover:-translate-y-1 transition-all">
 <div className="flex items-center justify-between">
 <h3 className="font-bold text-base text-[#10201C] ">{dept.name}</h3>
 <span className="px-2.5 py-1 rounded-full text-[10px] font-extrabold uppercase bg-[#E6F1EE] text-[#053229] border border-[#B8CCC5] ">
 SLA: {dept.avgResolutionTime}h
 </span>
 </div>

 <div className="space-y-2 text-xs text-[#536761] font-medium">
 <div className="flex justify-between">
 <span>Active Issues:</span>
 <strong className="text-[#10201C] ">{openDept} / {deptIssues.length}</strong>
 </div>
 <div className="flex justify-between">
 <span>SLA Escalations:</span>
 <strong className={escDept > 0 ? 'text-rose-600 font-bold' : 'text-[#10201C] '}>
 {escDept}
 </strong>
 </div>
 <div className="flex justify-between">
 <span>Fix Rate:</span>
 <strong className="text-emerald-600 ">{resRate}%</strong>
 </div>
 </div>
 ...
 <div
 className="h-full rounded-full bg-gradient-to-r from-[#07483A] to-emerald-400 transition-all duration-500"
 style={{ width: `${resRate}%` }}
 />
 </div>
 </div>
