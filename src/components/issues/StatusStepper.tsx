...
export const StatusStepper: React.FC<StatusStepperProps> = ({ status, statusHistory }) => {
 ...
 <motion.div
 className="absolute left-0 top-1/2 -translate-y-1/2 h-1 bg-gradient-to-r from-[#07483A] via-[#0B6652] to-emerald-400 rounded-full z-0"
 initial={{ width: '0%' }}
 animate={{
 ...
 isCompleted
 ? 'bg-[#053229] border-[#0B6652] text-white shadow-lg shadow-[#053229]/40'
 : isCurrent
 ? 'bg-slate-900 border-amber-400 text-amber-400 ring-4 ring-amber-400/20'
 : 'bg-slate-900 border-slate-700 text-[#536761]'
 }`}
 >
 {isCompleted ? (
 ...
 isCompleted ? 'text-[#B8CCC5]' : isCurrent ? 'text-amber-400 font-bold' : 'text-[#536761]'
 }`}
 >
 {step}
 </span>
 ...
 {historyMatch && (
 <span className="text-[10px] text-[#536761] font-medium">
 ...
 <h5 className="text-xs font-bold uppercase tracking-wider text-[#73827D] flex items-center gap-1.5">
 <FileText className="w-3.5 h-3.5 text-[#0B6652]" /> Resolution Workflow History
 </h5>
 <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
 {statusHistory.map((item, index) => (
 <div key={index} className="p-2.5 rounded-xl bg-slate-900/60 border border-slate-800/60 text-xs">
 <div className="flex items-center justify-between text-slate-300">
 <span className="font-semibold text-[#B8CCC5]">{item.status}</span>
 <span className="text-[10px] text-[#73827D]">{formatRelativeTime(item.timestamp)}</span>
 </div>
 <p className="text-[11px] text-[#73827D] mt-1">
